...
export function getPhoneReviewData(phone: PhoneSpec): PhoneReview {
  ...
  if (isFlagship) {
    ...
    const passportTaxStr = phone.ptaTax?.passportTaxPKR ? `Rs ${phone.ptaTax.passportTaxPKR.toLocaleString()} on passport` : 'statutory passport slabs';
    const cnicTaxStr = phone.ptaTax?.cnicTaxPKR ? `Rs ${phone.ptaTax.cnicTaxPKR.toLocaleString()} on CNIC` : 'statutory CNIC slabs';
    pakistanMarketContext = `In Pakistan, the ${phone.name} retails at around Rs ${phone.pricePKR.toLocaleString()} official box-pack price with official warranty via local channels. For imported non-PTA units, the DIRBS customs registration slab adds approximately ${passportTaxStr} or ${cnicTaxStr}. While official distributor stock is readily available in major commercial hubs like Hafeez Centre (Lahore), Saddar (Karachi), and Blue Area (Islamabad), street cash discounts typically range between 3% to 6% below list price.`;
    performanceAndCamera = `Powered by the ${phone.specs.processor}, the device effortlessly handles intensive sustained loads including high frame rate gaming, high-bitrate 4K UHD video scrubbing, and heavy multitasking without thermal throttling. The camera setup with ${phone.specs.mainCamera.split('+')[0].trim()} and advanced stabilization delivers studio-grade dynamic range and class-leading low-light fidelity under difficult lighting conditions.`;
