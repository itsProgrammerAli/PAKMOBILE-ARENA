...
export type NavbarActiveView = 'home' | 'brand' | 'phone-detail' | 'compare' | 'pta-tax' | 'news';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  // Close desktop brands dropdown on outside click without prematurely swallowing item clicks
  useEffect(() => {
          ...
          <nav className="hidden md:flex items-center space-x-6 lg:space-x-8">
            ...
            <button
              ...
              className={`text-sm font-semibold transition-all py-1.5 relative cursor-pointer ${
                activeView === 'home' && (!selectedBrand || selectedBrand === 'all')
            ...
            <div className="relative" ref={brandsDropdownRef}>
              ...
              >
                <span>{selectedBrand && selectedBrand !== 'all' ? selectedBrand : 'Brands'}</span>
                <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${
                  isBrandsDropdownOpen ? 'rotate-180 text-emerald-600 dark:text-emerald-400' : selectedBrand && selectedBrand !== 'all' ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 dark:text-zinc-500 group-hover:text-gray-600 dark:group-hover:text-zinc-300'
                }`} />
                {selectedBrand && selectedBrand !== 'all' && (
                  <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-emerald-600 dark:bg-emerald-400 rounded-full" />
                )}
              ...
              {isBrandsDropdownOpen && (
                <div 
                  ...
                  className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 p-2 rounded-2xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 shadow-2xl z-50 pointer-events-auto animate-in fade-in slide-in-from-top-1 duration-150"
                >
                  <div className="px-3 py-1.5 text-[11px] font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider flex items-center justify-between border-b border-gray-100 dark:border-zinc-800 mb-1 pointer-events-none">
                    <span>Explore Brands</span>
                    <Layers className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                  </div>
                  <div className="grid grid-cols-2 gap-1 py-1 pointer-events-auto">
                    {BRANDS.map((b) => (
                      <button
                        key={b.slug}
                        ...
                        className={`flex items-center justify-between px-2.5 py-2 text-xs font-medium rounded-xl transition-colors text-left cursor-pointer pointer-events-auto select-none ${
                      ...
                      >
                        <span className="pointer-events-none">{b.name}</span>
                        <span className="text-[10px] text-gray-400 dark:text-zinc-500 font-mono pointer-events-none">{b.count}</span>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            >
              <Newspaper className="w-4 h-4 text-emerald-600 dark:text-emerald-400 group-hover:scale-110 transition-transform" />
              <span>News</span>
              {activeView === 'news' && (
                <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-emerald-600 dark:bg-emerald-400 rounded-full" />
              )}
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            >
              <div className="flex items-center gap-1.5 truncate">
                ...
                <span className="truncate">PTA Tax</span>
              </div>
              <span className="text-[9px] text-emerald-600 dark:text-emerald-400 font-bold shrink-0">DIRBS</span>
            </button>
          </div>
