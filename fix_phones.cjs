...
const parts = content.split('export const PHONES_DATA: PhoneSpec[] = ');
if (parts.length === 2) {
    let phonesStr = parts[1];
    // strip the trailing semicolon if it exists
    if (phonesStr.endsWith(';\n')) {
        phonesStr = phonesStr.slice(0, -2);
    } else if (phonesStr.endsWith(';')) {
        phonesStr = phonesStr.slice(0, -1);
    }
    
    let phones = JSON.parse(phonesStr);
    ...
    const newContent = parts[0] + 'export const PHONES_DATA: PhoneSpec[] = ' + JSON.stringify(phones, null, 2) + ';\n';
